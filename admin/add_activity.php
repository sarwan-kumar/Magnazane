...
if (isset($_GET['did'])) {
    ...
    $queryst = "SELECT * from add_activity where id='$did'"; 
    $resultst = mysqli_query($conn, $queryst);
    $firmsst = mysqli_fetch_array($resultst);
     //get image path
    $imageUrl = 'activity/'.$firmsst['img'];
   //echo $imageUrl; die;
    if(file_exists($imageUrl)){
        $msg='3';
      //delete the image
        unlink($imageUrl); 
    $sql = 'DELETE From add_activity WHERE id="' . $did . '"'; 
    $conn->query($sql);
...
if (isset($_POST['submit'])) {
    ...
    if (empty($imgFile)) {
        ...
        $upload_dir = 'activity/'; // upload directory
    ...
    $sql = "INSERT INTO `add_activity`(`name`, `img`, `status`, `date`) 
...
<div class="page-wrapper">
    <div class="page-content">
            ...
            <div class="ps-3">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Add Activity Images</li>
                    </ol>
                </nav>
            </div>
            <div class="ms-auto">
                ...
                <div class="card border-top border-0 border-4 border-info">
                    <div class="card-body p-5">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-info">Add Activity Image</h5>
                        </div>
                        <hr>
                        <form name="form1" action="add_activity.php" class="row g-3" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-12">
                                <button name="submit" type="submit" id="" class="btn btn-info px-5">Add Activity</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <!--end row-->
        <div class="row">
            <h6 class="mb-0 text-uppercase">All Activity images</h6>
            <hr />
            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-4 row-cols-xl-4">
            <?php  $query = "SELECT * FROM add_activity ";
            $result = $conn->query($query);
            $i = '1'; while ($firm = mysqli_fetch_array($result)) {  ?>
                <div class="col">
                    <div class="card border-success border-bottom border-3 border-0">
                        <img src="activity/<?php echo $firm['img']; ?>" class="card-img-top" alt="<?php echo $firm['name']; ?>" style="height: 260px;">
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <a href="add_activity.php?did=<?php echo $firm['id']; ?>" class="btn btn-danger"><i class="lni lni-trash"></i></a>
                            </div>
                        </div>
                    </div>
                </div>  <?php $i++;  } ?>
 ...
 <script>
    $(document).ready(function() {
       ...
       // error_staff(ff1);
        ...
        var ff1="Gallery Image Deleted Successful";
        msgBox_yellow(ff1);
